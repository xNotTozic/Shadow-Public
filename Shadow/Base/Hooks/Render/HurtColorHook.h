...
UIColor* HurtColorDetour(void* _this, UIColor* color, void* a3) // Actor*, Vector3*
{
	if (getModuleByName("hurtcolor")->isEnabled()) {
		UIColor RGB = ColorUtils::RainbowDark(1);
		color->r = RGB.r;
		color->g = RGB.g;
		color->b = RGB.b;
		color->a = RGB.a;
	...
	Utils::CallFunc<void*, void*, UIColor*, void*>( // CallFunc to call the original.
...
public:
	...
	bool Initialize() override // Now override the Initialize function so we can actually hook our stuff
	{
		...
		void* HurtColorAddr = Utils::findSig("E8 ? ? ? ? E9 ? ? ? ? 8B 43 ? 48 8D 54 24");
		...
		return Utils::HookFunction((void*)HurtColorFunc, (void*)&HurtColorDetour, &onHurtColor);
